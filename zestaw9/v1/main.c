...
pthread_mutex_t waitingForStart;
...
int main(int argc, char ** argv) {
    ...
    initializeSharedArray(atoi(argv[5]));
...
void createThreads() {
    ...
    pthread_mutex_init(&waitingForStart, &attr);
    pthread_mutex_init(&mutex, &attr);

    pthread_mutex_lock(&waitingForStart);
    for(int i = 0; i < readersSize; i++){
    ...
    pthread_mutex_unlock(&waitingForStart);
